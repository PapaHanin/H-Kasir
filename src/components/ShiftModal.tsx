        ...
        <div className="p-6 space-y-4">
          {!activeShift ? (
            ...
            <form onSubmit={handleStartShiftSubmit} className="space-y-4">
              <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-700 dark:text-emerald-300 font-mono">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-3 py-2.5 rounded-xl border text-base font-bold font-numeric bg-slate-50 dark:bg-slate-950 border-slate-300 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    required
                  />
                </div>
                <div className="flex gap-2 mt-2">
                  {[100000, 200000, 300000, 500000].map((amt) => (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg text-xs font-bold font-numeric bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700"
                    >
                      {formatRupiah(amt)}
                    </button>
                  ))}
            ...
            <form onSubmit={handleCloseShiftSubmit} className="space-y-4">
              ...
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 space-y-2 text-xs">
                ...
                <div className="flex justify-between font-mono">
                  ...
                  <span className="font-bold text-emerald-600 dark:text-emerald-400 font-numeric">{formatRupiah(currentShiftStats.cashSales)}</span>
                </div>
                <div className="flex justify-between font-mono">
                  <span className="text-slate-500">Total Non-Tunai (QRIS/E-Wallet):</span>
                  <span className="font-bold text-blue-600 dark:text-blue-400 font-numeric">{formatRupiah(currentShiftStats.nonCashSales)}</span>
                </div>
                <div className="border-t border-slate-200 dark:border-slate-800 pt-2 flex justify-between font-bold text-sm font-mono">
                  ...
                  <span className="text-emerald-600 dark:text-emerald-400 font-numeric">{formatRupiah(currentShiftStats.expectedCash)}</span>
                </div>
              </div>
              ...
              <div className="pt-3 flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-black text-xs rounded-xl shadow-md tracking-wide"
                >
